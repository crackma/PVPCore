...
public class CategoryCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "additem":
                ...
                try {
                    category.addItem(new CategoryItem(itemStack, args[2], Integer.parseInt(args[3]), Integer.parseInt(args[4])));
                } catch (NumberFormatException exception) {
                    return false;
                }
                ShopDatabase.insertCategory(category);
    ...
    public List<String> onTabComplete(CommandSender commandSender, Command command, String alias, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            //category edit <name> displayItem/inventorySlot/description <new>
            case "edit":
                if (args.length == 2) ShopManager.getCategorySet().forEach(category -> completions.add(category.getName()));
                if (args.length == 3) {
